from langchain.llms import OpenAI  # Import OpenAI from the right module
...
def generate_emails(subject, tone):
    ...
    prompt_template_name = PromptTemplate(
        ...
        template=f"generate a brief e-mail on the subject {subject} and make sure the tone is {tone} in 1 line only"
    )

    name_chain = LLMChain(llm=llm, prompt=prompt_template_name, output_key="text")

    response = name_chain({"subject": subject, "tone": tone})  # Correct the key
    return response
